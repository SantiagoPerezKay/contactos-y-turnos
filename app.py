...
app.config['MYSQL_PASSWORD']='asd1234'
...
def login():
    ...
    if request.method == 'POST':
        ...
        cur.execute(f'SELECT adminname FROM admins WHERE adminname="{username1}" AND adminpassword="{password1}" ')
...
def agregar_contacto():
    ...
    if request.method =='POST':
        ...
        if len(_nombrecompleto)<=60 and len(str(-_telefono))<=12 and len(_email)<=60:
            ...
            cur = mysql.connection.cursor()
            cur.execute('INSERT INTO contactos (id,nombrecompleto,telefono,email,fechacreacion) VALUES (NULL,%s,%s,%s,%s)',(_nombrecompleto,_telefono,_email,now))
...
def turnos():
    ...
    if 'username' in session :
        now=datetime.now()
        cur=mysql.connection.cursor()
        cur.execute(f'select * from turnos WHERE fechaturno>="{now.date()}" ORDER BY fechaturno ASC,desde ASC LIMIT 20 ')
        data=cur.fetchall()
...
def mandar_whatapp(contactonombrecompleto):
    ...
    cur=mysql.connection.cursor()
    cur.execute(f'select telefono from contactos WHERE nombrecompleto="{contactonombrecompleto}"')
    data=cur.fetchone()
    cur.execute(f'select fechaturno from turnos WHERE contactonombrecompleto="{contactonombrecompleto}"')
    data2=cur.fetchone()

    mandar_mensaje("+54"+str(data[0]),f"Aviso recordatorio de su turno para el dia:{str(data2[0])}")
...
def agregar_turno():
    ...
    if request.method =='POST' and 'username' in session:
        ...
        cur = mysql.connection.cursor()
        cur.execute('INSERT INTO turnos (idturno,contactonombrecompleto,fechacreacionturno,fechaturno,desde,hasta,observaciones) VALUES (NULL,%s,%s,%s,%s,%s,%s)',(contactonombrecompleto,fechacreacionturno,fechaturno,desde_str,hasta,observaciones))
        mysql.connection.commit()
...
def mandar_whatapps():
    if 'username' in session :
        ...
        cur.execute(f'SELECT contactonombrecompleto FROM turnos WHERE fechaturno="{fecha_siguiente}"')
        nombres_tunos=cur.fetchall()
        

        for nombre in nombres_tunos:
            cur.execute(f'SELECT telefono FROM contactos WHERE nombrecompleto="{nombre[0]}"')
            telefono=cur.fetchone()
           
            mandar_mensaje("+54"+str(telefono[0]),f"automensaje:aviso recordatorio de su turno para el dia {fecha_siguiente}")
